...
enum class state {EMPTY, ALG_1, ALG_2,};
...
public:
    ...
    Intersection(): status(state::EMPTY) {}
    Intersection(string lbl) :
    label(lbl), status (state::EMPTY) {}
};
...
public:
    ...
    Board()
    {
        ...
        void placePiece(string pos, state player){
            int sIdx= pos[1]- '0';
            int iIdx= pos[3]- '0';
...
for (int i; i<cow; ++i){
    ...
    cout<< "Alg_1 move:";
    cin>>move1;
    b.placePiece(move1, state::ALG_1);
    output<< "Alg_100-"<<move1<<endl;
    cout<<"Alg_2 move:";
    cin>>move2;
    b.placePiece(move2, state::ALG_2);
